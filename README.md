...
4. After installing the required libraries, we will be able to start our Node/Express server. To start our server, go back to your terminal (that is currently inside the **back** folder directory) and type the command **node server.js**. If you followed all the previous steps correctly, when you run the command you should see the following output in your terminal, **Server running on http://localhost:3000**. You have now completed most of the required coding part for this demo. We still need to include our AWS information, but we can do this after the following sections. Good work!
